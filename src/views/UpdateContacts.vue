      ...
      <v-row justify="center">
        <v-col cols="md-4">
          <v-card color="hsl(0, 38%, 98%)" v-if="!contactId">
            ...
            ></v-progress-linear>
            <v-card-title class="justify-center text--secondary font-weight-bold" v-text="heading1"></v-card-title>
            ...
            <v-form ref="form" @submit.prevent="addContact">
              ...
              <v-card-actions>
                <v-container>
                  <v-row>
                    <v-col>
                      <v-btn class="ma-1" dark color="#f0ad94"  :type="'submit'" depressed>Submit</v-btn>
                    </v-col>
                  </v-row>
                </v-container>
              </v-card-actions>
          ...
          <v-card color="hsl(0, 38%, 98%)" v-else>
            <v-progress-linear
              :active="loading"
              :indeterminate="loading"
              absolute
              ...
              color="deep-purple accent-4"
            ></v-progress-linear>
            <v-card-title class="justify-center text--secondary font-weight-bold" v-text="heading2"></v-card-title>
            <v-snackbar :color="snackbarBcg" v-model="snackbar">
              {{message}}
              <template v-slot:action="{ attrs }">
                <v-btn v-bind="attrs" @click="snackbar = false">Close</v-btn>
